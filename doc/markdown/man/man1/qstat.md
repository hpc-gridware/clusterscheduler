...
options allow you to get information about specific jobs, queues or users. If multiple selections are done a queue 
...
reason for the alarm state. Suspend alarm state reasons will be displayed by *A*. *E* displays the reason for a 
...
`qstat -s` *huhohshdhjha* and `qstat -s` a is an abbreviation for `qstat -s` *psr* (see `-a`, `-hold_jid` and 
...
*OUTPUT FORMATS* sub-section *Reduced Format* below for detailed information. Sub-tasks of parallel jobs should not 
