...
13. Choose Spooling Method: Select `classic`, `berkeleydb`, or `postgres`
    spooling. As part of this step, the spooling tables (or directories,
    for classic) will be created.
    ...
    If you select `postgres`, the installer asks for the PostgreSQL
    connection parameters (host, port, database name, role, optional
    SSL mode, optional `.pgpass` setup). The PostgreSQL database and
    the role qmaster connects as must already exist — see the
    dedicated chapter **PostgreSQL Spooling** for the full
    provisioning workflow, role-permission requirements, and the
    `.pgpass` convention.
...
2. Make a copy of the configuration template file and prepare it with the answers to the questions that are usually asked during the manual installation process. If the root user has no write permissions in $ SGE_ROOT, then choose a different path but make sure that you preserve the file for the uninstallation process.
   ...
   If you set `SPOOLING_METHOD="postgres"` in the template, also
   fill in the `SPOOLING_PG_*` variables (host, port, database,
   user, optional SSL mode, optional `.pgpass` path and password)
