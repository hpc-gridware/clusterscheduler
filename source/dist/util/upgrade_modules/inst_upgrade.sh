...
{
   $INFOTEXT -u "\nWelcome to the Cluster Scheduler Upgrade Procedure"
...
GetBackupDirectory()
{
   ...
   while [ $done = false ]; do
      $CLEAR
      $INFOTEXT -u "\nType the complete path to the Cluster Scheduler configuration backup directory."
      $INFOTEXT "\nThis directory must have been generated by running the save_config.sh\nscript in the old cluster.\n"
      ...
      if [ -n "$version" -a -n "$backup_date" -a -d "${UPGRADE_BACKUP_DIR}/cell" ]; then
         #Ask if correct
	      $INFOTEXT -n "\nFound backup from version"
	      $INFOTEXT -n "\n$version"
	      $INFOTEXT -n "\ncreated on $backup_date"
