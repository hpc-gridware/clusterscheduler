...
CheckConfigFile()
{
   ...
   if [ "$QMASTER" = "install" ]; then
      ...
      # A config file without this entry was derived from the template of an
      # older version. Keep the old behaviour and create @allhosts.
      echo "$CONFIG_ENTRIES" | grep "^CREATE_ALLHOSTS_HOSTGROUP\$" > /dev/null 2>&1
      if [ $? != 0 ]; then
         $INFOTEXT -log "Your autoinstall config file has no >CREATE_ALLHOSTS_HOSTGROUP< entry, using >true<"
         CREATE_ALLHOSTS_HOSTGROUP="true"
      elif [ -z "$CREATE_ALLHOSTS_HOSTGROUP" ]; then
         $INFOTEXT -e "Your >CREATE_ALLHOSTS_HOSTGROUP< flag is not set!"
         $INFOTEXT -log "Your >CREATE_ALLHOSTS_HOSTGROUP< flag is not set!"
         is_valid="false"
      fi
      if [ "$CREATE_ALLHOSTS_HOSTGROUP" = "1" ]; then
         CREATE_ALLHOSTS_HOSTGROUP="true"
      elif [ "$CREATE_ALLHOSTS_HOSTGROUP" = "0" ]; then
         CREATE_ALLHOSTS_HOSTGROUP="false"
      fi
      CREATE_ALLHOSTS_HOSTGROUP=`echo $CREATE_ALLHOSTS_HOSTGROUP | tr "[A-Z]" "[a-z]"`
      if [ "$CREATE_ALLHOSTS_HOSTGROUP" != "true" -a "$CREATE_ALLHOSTS_HOSTGROUP" != "false" ]; then
         $INFOTEXT -e "Your >CREATE_ALLHOSTS_HOSTGROUP< flag is wrong! Valid values are: 0, 1, true, false"
         $INFOTEXT -log "Your >CREATE_ALLHOSTS_HOSTGROUP< flag is wrong! Valid values are: 0, 1, true, false"
         is_valid="false"
      fi
   fi
...
#-------------------------------------------------------------------------
# SetupRcScriptNames: Sets up RC script name variables
# $1 ... hosttype (master|execd|dbwriter)
...
ProcessSGEClusterName()
{
   ...
   #Only qmaster installation can create cluster_name file
   if [ \( "$1" = "qmaster" -o "$UPDATE" = "true" \) -a ! -f $SGE_ROOT/$SGE_CELL/common/cluster_name ]; then
