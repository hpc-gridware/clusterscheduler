...
#              endif
               ...
               job_elem = (job_elem_t *)sge_malloc(sizeof(job_elem_t));
               job = &job_elem->job;
...
get_arsess_list(lnk_link_t *arsess_list)
{
   ...
   if (ashes == NULL) {
      ash_max = ASHMAXINC;
      ashes = (ash_t *)sge_malloc(sizeof(ash_t)*ash_max);
   ...
   if (num_ashes > 0) {
      for (i=0; i<num_ashes; i++) {
         if (syssgi(SGI_GETARSESS, &ashes[i], &ar) >= 0) {
            ...
            arse_elem = sge_malloc(sizeof(arsess_elem_t));
