...
sge_get_file_priv_key(void)
{
   ...
   DRETURN(sge_dstring_get_string(&priv_key));
}
...
buffer_read_from_stdin(char **buffer, size_t *size) 
{
   ...
   while (repeat) {
      ...
      if (buffer_length <= 0) {
         ...
         if (buffer_ptr == NULL) {
            buffer_ptr = sge_malloc(buffer_length + 1);
         } else {
               ...
               size_t *buffer_out_length)
{
   ...
   ekey[0] = sge_malloc(shared_ssl_func__EVP_PKEY_size(pubKey[0]));  
   if (!ekey[0]) {
...

{
   ...
   encryptKey = sge_malloc(sizeof(char) * ekeylen);
   if (!encryptKey) {
      shared_ssl_func__EVP_PKEY_free(privateKey);
      error_code = shared_ssl_func__ERR_get_error();
      shared_ssl_func__ERR_error_string(error_code, err_msg);
