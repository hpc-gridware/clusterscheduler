...
hgroup_add_del_mod_via_gdi(lListElem *this_elem, lList **answer_list, u_long32 gdi_command)
{
   ...
   if (this_elem != nullptr) {
      ...
      gdi_answer_list = sge_gdi(ocs::GdiTarget::Target::SGE_HGRP_LIST, gdi_command, &hgroup_list, nullptr, nullptr);
...
hgroup_get_via_gdi(lList **answer_list, const char *name)
{
   ...
   if (name != nullptr) {
      ...
      gdi_answer_list = sge_gdi(ocs::GdiTarget::SGE_HGRP_LIST, SGE_GDI_GET, &hostgroup_list, where, what);
      lFreeWhat(&what);
      lFreeWhere(&where);
...
bool hgroup_show_structure(lList **answer_list, const char *name, bool show_tree)
{
   ...
   if (name != nullptr) {
      ...
      what = lWhat("%T(ALL)", HGRP_Type);
      alp = sge_gdi(ocs::GdiTarget::SGE_HGRP_LIST, SGE_GDI_GET, &hgroup_list, nullptr, what);
      lFreeWhat(&what);
