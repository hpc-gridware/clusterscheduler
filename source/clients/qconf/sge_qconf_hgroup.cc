...
bool 
hgroup_add_del_mod_via_gdi(lListElem *this_elem, lList **answer_list, ocs::GdiCommand::Command gdi_command)
{
   bool ret = true;
   ...
   if (this_elem != nullptr) {
      ...
      gdi_answer_list = sge_gdi(ocs::GdiTarget::Target::SGE_HGRP_LIST, gdi_command, ocs::GdiSubCommand::SGE_GDI_SUB_NONE ,&hgroup_list, nullptr, nullptr);
...
hgroup_get_via_gdi(lList **answer_list, const char *name)
{
   ...
   if (name != nullptr) {
      ...
      gdi_answer_list = sge_gdi(ocs::GdiTarget::SGE_HGRP_LIST, ocs::GdiCommand::SGE_GDI_GET, ocs::GdiSubCommand::SGE_GDI_SUB_NONE, &hostgroup_list, where, what);
      lFreeWhat(&what);
      lFreeWhere(&where);
...
hgroup_add(lList **answer_list, const char *name, bool is_name_validate ) {
   ...
   if (name != nullptr) {
      ...
      if (ret) {
         ret = hgroup_add_del_mod_via_gdi(hgroup, answer_list, ocs::GdiCommand::SGE_GDI_ADD);
      }
...
bool hgroup_modify(lList **answer_list, const char *name) {
   ...
   if (name != nullptr) {
      ...
      if (ret) {
         ret = hgroup_add_del_mod_via_gdi(hgroup, answer_list, ocs::GdiCommand::SGE_GDI_MOD);
      }
      lFreeElem(&hgroup);
   }
...
hgroup_delete(lList **answer_list, const char *name)
{
   ...
   if (name != nullptr) {
      ...
      if (hgroup != nullptr) {
         ret = hgroup_add_del_mod_via_gdi(hgroup, answer_list, ocs::GdiCommand::SGE_GDI_DEL);
      }
      lFreeElem(&hgroup);
   }
   DRETURN(ret);
...
bool hgroup_show_structure(lList **answer_list, const char *name, bool show_tree)
{
   ...
   if (name != nullptr) {
      ...
      what = lWhat("%T(ALL)", HGRP_Type);
      alp = sge_gdi(ocs::GdiTarget::SGE_HGRP_LIST, ocs::GdiCommand::SGE_GDI_GET, ocs::GdiSubCommand::SGE_GDI_SUB_NONE, &hgroup_list, nullptr, what);
      lFreeWhat(&what);
