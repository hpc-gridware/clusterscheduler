...
qstat_show_job(lList *jid_list, u_long32 isXML, qstat_env_t *qstat_env) {
   ...
   alp = sge_gdi(ocs::GdiTarget::Target::SGE_SME_LIST, ocs::GdiCommand::SGE_GDI_GET, ocs::GdiSubCommand::SGE_GDI_SUB_NONE, &ilp, nullptr, what);
   lFreeWhat(&what);
...
static int qstat_show_job_info(u_long32 isXML, qstat_env_t *qstat_env)
{
   ...
   /* get job scheduling information */
   what = lWhat("%T(ALL)", SME_Type);
   alp = sge_gdi(ocs::GdiTarget::SGE_SME_LIST, ocs::GdiCommand::SGE_GDI_GET, ocs::GdiSubCommand::SGE_GDI_SUB_NONE, &ilp, nullptr, what);
   lFreeWhat(&what);
   if (isXML){
