...
bool
ocs::QHostParameter::switch_list_qhost_parse_from_file(lList **switch_list, lList **answer_list, const char *file) {
   DENTER(TOP_LAYER);
   ...
   const bool ret = sge_parse_cmdline_qhost(token, environ, switch_list, answer_list);
   sge_strafree(&token);
...
bool
ocs::QHostParameter::sge_parse_cmdline_qhost(char **argv, char **envp, lList **ppcmdline, lList **alpp) {
   DENTER(TOP_LAYER);
   char **sp;
   char **rp;
...
int
ocs::QHostParameter::sge_parse_qhost(lList **ppcmdline, lList **alpp) {
   u_long32 helpflag;
...
bool ocs::QHostParameter::parse_parameters(lList **alpp, char **argv, char **envp) {
   ...
   switch_list_qhost_parse_from_file(&switches_default_files, alpp, sge_dstring_get_string(&file));
   ...
   int is_ok = sge_parse_qhost(&switches_cmd_line, alpp);
   lFreeList(&switches_cmd_line);
   if (is_ok == 0) {
      answer_list_output(alpp);
      sge_exit(1);
