...
void *tty_to_commlib(void *t_conf) {
   ...
   bool do_exit = false;
   while (!do_exit) {
      // @todo probably this thread should only start its processing once the commlib_to_tty thread received a
      //       REGISTER_CTRL_MSG from the sge_shepherd and replied to it
      //       We could wait on the wakeup_pipe for a byte sent from the commlib_to_tty thread.
      fd_set read_fds;
      FD_ZERO(&read_fds);
      ...
      // @todo what is the reason? I have seen a SIGCONT interrupting the pty_to_commlib thread in sge_shepherd
      //       which did *not* handle this.
      // ==> Probably handling a real suspend/resume, e.g., in the shell pressing CTRL-Z, later fg
...
void* commlib_to_tty(void *t_conf)
{
   ...
   while (do_exit == 0) {
      ...
      thread_testcancel(t_conf);
      // Why shall we do the check_window_change in both threads?
      // In one is enough.
      // client_check_window_change(g_comm_handle);
