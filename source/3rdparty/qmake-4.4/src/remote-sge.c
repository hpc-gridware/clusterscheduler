...
*                            (in dynamic mode: also -l, -q, and -soft/-hard)
...
static struct hostfile_info *read_hostinfo(void)
{
...
****************************************************************************
*/
static int get_host_count(void)
{
...
****************************************************************************
*/
static void create_hostfile(void)
{
   FILE* sge_hostfile;
...
static void lock_hostfile(void)
{
   int lockfile;
...
static void unlock_hostfile(void)
{
   if(hostfile_locked) {
      hostfile_locked = 0;
...
static const char *next_host(void)
{
   struct hostfile_info *hostinfo;
   off_t offset;
   off_t file_offset;
...
static void build_submit_argv(void)
{
   int i;
   int v_counter = 0;

   /* copy sge -v options for later calls to qrsh -inherit
    * for dynamic mode, we also have to copy job requests (-l) queue requests (-q)
    * and the -soft and -hard specifiers
    */
   for (i = 0; i < sge_argc; i++) {
      if (strcmp(sge_argv[i], "-V") == 0) {
         ...
         v_counter += 2;
         i++;
      } else if (dynamic_mode && strcmp(sge_argv[i], "-q") == 0) {
         v_counter +=2;
         i++;
      } else if (dynamic_mode && strcmp(sge_argv[i], "-hard") == 0) {
         v_counter++;
      } else if (dynamic_mode && strcmp(sge_argv[i], "-soft") == 0) {
         v_counter++;
      }
   }
   sge_v_argc = 0;
   ...
   for (i = 0; i < sge_argc; i++) {
      if (strcmp(sge_argv[i], "-v") == 0 && (i + 1) < sge_argc) {
         ...
         sge_v_argv[sge_v_argc++] = sge_argv[i];
      } else if (dynamic_mode && strcmp(sge_argv[i], "-l") == 0 && (i + 1) < sge_argc) {
         sge_v_argv[sge_v_argc++] = sge_argv[i++];
         sge_v_argv[sge_v_argc++] = sge_argv[i];
      } else if (dynamic_mode && strcmp(sge_argv[i], "-q") == 0 && (i + 1) < sge_argc) {
         sge_v_argv[sge_v_argc++] = sge_argv[i++];
         sge_v_argv[sge_v_argc++] = sge_argv[i];
      } else if (dynamic_mode && strcmp(sge_argv[i], "-hard") == 0) {
         sge_v_argv[sge_v_argc++] = sge_argv[i];
...
static int parse_options(int *p_argc, char **p_argv[])
{
   ...
   if(first_gmake_option == 0) {
      /* all parameters are for gmake - reset be_verbose */
      be_verbose = 0;
      if(jobid != NULL) {
         unhandled_recursive = 1;
      } else {
         /* continue and behave like gmake */
         return 0;
      }
   } else {
      /* found SGE parameters */
      if(be_verbose) {
         fprintf(stdout, "incoming parameters:\n");
         for(i = 0; i < argc; i++) {
            fprintf(stdout, "%ld: argv[%d] = %s\n", (unsigned long) getpid(), i, argv[i]);
         }
      }
   }

   if(unhandled_recursive) {
      /* case: might be recursive make call (we have job id) */
   ...
   /* determine dynamic_mode (do we have PE environment set?) */
   dynamic_mode = getenv("PE") == NULL;

   /* parse sge options and set some flags */
   for(i = 0; i < sge_argc; i++) {
      /* be_verbose already parsed at beginning */
      if(strcmp(sge_argv[i], "-cwd") == 0) {
         pass_cwd = 1;
      } else if(strcmp(sge_argv[i], "-v") == 0) {
         i++;
         if(i < sge_argc) {
   ...
   *p_argv = gmake_argv;
   ...
   if(be_verbose) {
      if (dynamic_mode) {
         fprintf(stdout, "dynamic task allocation mode (no parallel environment is set)\n");
      }
      fprintf(stdout, "resulting argv parameters:\n");
      for(i = 0; i < sge_argc; i++) {
         fprintf(stdout, "%ld: sge_argv[%d] = %s\n", (unsigned long) getpid(), i, sge_argv[i]);
      }
      for(i = 0; i < sge_v_argc; i++) {
         fprintf(stdout, "%ld: sge_v_argv[%d] = %s\n", (unsigned long) getpid(), i, sge_v_argv[i]);
      }
      for(i = 0; i < gmake_argc; i++) {
         fprintf(stdout, "%ld: gmake_argv[%d]  = %s\n", (unsigned long) getpid(), i, gmake_argv[i]);
      }
   }
...
static int inherit_job(void)
{
   ...
   for(i = 0; i < sge_argc; i++) {
      if(!strcmp(sge_argv[i], "-inherit")) {
         if(be_verbose) {
            fprintf(stdout, "removing -inherit option\n");
         }
         sge_argc--;
...
void set_default_options(void)
{
   ...
   if (insert_resource_request) {
      ...
      if (architecture == NULL || strlen(architecture) == 0) {
         ...
         fprintf(stdout, "           will not add default -l arch=<SGE_ARCH> request\n");
         return;
      }
      ...
      if (insert_resource_request) {
         ...
         if (be_verbose) {
            fprintf(stdout, "setting default options: -hard -l %s\n", buffer);
         }

         sge_argv[sge_argc++] = "-hard";
         sge_argv[sge_argc++] = "-l";
...
*/
#if 0
static void equalize_pe_j(void)
{
   int i;
   int nslots = 0;
   char **argv;
...
static void submit_qmake(void)
{
   ...
   if(qrsh_pid) {
      ...
      while(1) {
         int status;
         if(be_verbose) {
            fprintf(stdout, "parent(%ld): wait for child exit ...\n", (unsigned long) getpid());
         }
         if(waitpid(qrsh_pid, &status, 0) == qrsh_pid) {
            if(WIFEXITED(status)) {
               int exit_status = WEXITSTATUS(status);
               if(be_verbose) {
                  fprintf(stdout, "parent(%ld): now terminating with obtained child exit status %d from child\n", (unsigned long) getpid(), exit_status);
               }
               exit(exit_status);
            }
      ...
      /* in child, start qrsh */
      if(be_verbose) {
         fprintf(stdout, "child: starting scheduled qmake\n");
         for(i = 0; i < argc; i++) {
            fprintf(stdout, "child %ld: startup_args[%3d] = %s\n", (unsigned long) getpid(), i, argv[i]);
         }
      }
...
#endif
      if(be_verbose) {
         fprintf(stdout, "starting scheduled qmake ...\n");
      }
                      ...
                      int *id_ptr, int *used_stdin)
{
   ...
   if(child_pid) {
      ...
      *id_ptr = child_pid;
      ...
      if (exec_remote) {
         if (!dynamic_mode) {
            startup_mode = "qrsh remote execution (use -inherit)";
         } else {
            startup_mode = "qrsh remote execution (dynamic mode)";
         }
         args[argc++] = "qrsh";
         ...
         /* add the task specific requests from SGE_RREQ as last arguments */
         if (dynamic_mode) {
            if (resource_request) {
      ...
      if(be_verbose) {
         fprintf(stdout, "child: starting job (%s):\n", startup_mode);
         for(i = 0; args[i] != NULL; i++) {
            fprintf(stdout, "child %ld: startup_args[%3d] = %s\n", (unsigned long) getpid(), i, args[i]);
         }
      }
...
static int read_remote_status(int *exit_code_ptr, int *signal_ptr, int *coredump_ptr, int block)
{
   ...
   *coredump_ptr  = 0;
   ...
   if(child_pid <= 0) {
      if(be_verbose) {
         if (block == 0 && my_errno == 0) {
            fprintf(stdout, "parent(%ld): no child finished recently\n", (unsigned long) getpid());
         }
         if (my_errno != 0) {
            fprintf(stderr, "parent(%ld): waiting for child failed: %s\n",  (unsigned long) getpid(), strerror(my_errno));
         }
      }
      return child_pid;
   }
   ...
   if(be_verbose) {
      fprintf(stdout, "parent(%ld): child %ld finished\n",
                      (unsigned long)getpid(), (unsigned long)child_pid);
   }
   return child_pid;
}
