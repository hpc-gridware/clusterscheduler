...
sge_env_state_class_create(const char *sge_root, const char *sge_cell, int sge_qmaster_port, int sge_execd_port,
   ...
   if (!sge_env_state_setup(ret, sge_root, sge_cell, sge_qmaster_port, sge_execd_port, from_services, qmaster_internal,
                            eh)) {
      sge_env_state_class_destroy(&ret);
      DRETURN(NULL);
   }
...
static bool
sge_env_state_setup(sge_env_state_class_t *thiz, const char *sge_root, const char *sge_cell, u_long32 sge_qmaster_port,
                    u_long32 sge_execd_port, bool from_services, bool qmaster_internal, sge_error_class_t *eh) {
   DENTER(TOP_LAYER);
