...
static int cl_com_ssl_connection_request_handler_setup_finalize(cl_com_connection_t* connection) {
    ...
    * ("failed sending task to execd ...: can't find connection"). This is the
    * TLS/SSL counterpart of the same fix in cl_tcp_framework.cc. Use the kernel
    * maximum (SOMAXCONN); the kernel silently caps it to its own limit. */
