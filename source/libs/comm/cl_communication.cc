...
static int cl_com_dup_host(char **host_dest, const char *source, cl_host_resolve_method_t method, const char *domain) {
   ...
   switch (method) {
      case CL_SHORT:
         if ((the_dot = strchr((char *)source, '.')) != NULL) {
...
int cl_com_connection_complete_request(cl_raw_list_t *connection_list, cl_connection_list_elem_t *elem, long timeout,
   ...
   if (do_read_select) {
      ...
      if (connection->connection_sub_state == CL_COM_READ_CM) {
         ...
         if ((retval = cl_com_cached_gethostbyname(cm_message->dst->comp_host, &unique_host, &tmp_addr, NULL, NULL)) !=
             CL_RETVAL_OK) {
         ...
         if (cm_message->rdata != NULL) {
            if ((retval = cl_com_cached_gethostbyname(cm_message->rdata->comp_host, &unique_host, &tmp_addr, NULL,
                                                      NULL)) != CL_RETVAL_OK) {
