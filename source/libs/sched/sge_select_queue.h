...
typedef struct {
   ...
   lList      *rsmap_job_keys;    // for a scope=job request: the instance group the whole job is
                                  // bound to, per resource map (VA_Type, VA_variable is the map
                                  // name, VA_value the group key) - see rsmap_job_scope_key()
