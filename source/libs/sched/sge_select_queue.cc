 ...
 *
 * The limit list, the two skip lists, the groups scope=job requests are bound to and the chosen
 * resource map instances are only valid for one point in time, so they have to go whenever the
 * assignment is retried at a different one.
 *
 * @param[in,out] a the assignment
 */
 ...
 */
bool sge_select_queue(lList *requested_attr, lListElem *queue, lListElem *host,
...
int sge_load_alarm(char *reason, size_t reason_size, const lListElem *qep, const lList *threshold,
 ...
 * @return 0 on success, -1 on an error in a called function
 */
int sge_split_cal_disabled(bool monitor_next_run, lList **queue_list, lList **disabled) {
   DENTER(TOP_LAYER);
 ...
 * @return 0 on success, -1 on an error in a called function
 */
int sge_split_disabled(bool monitor_next_run, lList **queue_list, lList **disabled) {
   DENTER(TOP_LAYER);
...
ri_time_by_slots(const sge_assignment_t *a, lListElem *rep, const lList *load_attr, const lList *config_attr,
   ...
   if (ready_time == DISPATCH_TIME_QUEUE_END) {
      ...
      if (threshold < 0) {
         ...
         const bool props_given =
            is_rsmap && centry_rsmap_resolve_request_properties(rep, a->centry_list,
                                                                &required_props);
         if (same_given || id_given || props_given) {
            ...
            uint64_t group_when = utilization_rsmap_below(capacitiy_el, actual_el, same_arg,
                                                          group_amount, id_arg, required_props);
...
rsmap_job_scope_key(sge_assignment_t *a, const lListElem *req, const char *same_key,
   ...
   for (int pass = 0; pass < 2 && totals.empty(); ++pass) {
      ...
      for_each_ep_lv(host, a->host_list) {
         ...
         for_each_ep_lv(key_ep, keys) {
            const char *key = lGetString(key_ep, ST_name);
            const uint32_t free = centry_rsmap_group_free(definition, taken, same_key, key,
