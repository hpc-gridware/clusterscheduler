...
#include "msg_qstat.h"
#include "ocs_QStatParameterClient.h"
...
bool
ocs::QStatParameterClient::switch_list_qstat_parse_from_cmdline(lList **ppcmdline, lList **answer_list, char **argv) {
   DENTER(TOP_LAYER);
   bool ret = true;
   char **sp;
   stringT str;
...
ocs::QStatParameterClient::sge_parse_qstat(lList **ppcmdline, lList **ppljid) {
   ...
   */
   ...
   while (lGetNumberOfElem(*ppcmdline)) {
      ...
      while (parse_multi_stringlist(ppcmdline, "-pe", &alp, &pe_ref_list_, ST_Type, ST_name)) {
         need_queues_ = true;
      }

      while (parse_multi_stringlist(ppcmdline, "-q", &alp, &queue_ref_list_, QR_Type, QR_name)) {
         need_queues_ = true;
      }
