...
int sge_gdi_ctx_class_prepare_enroll(lList **answer_list) {
...
sge_setup2(int progid, u_long32 thread_id, lList **alpp, bool is_qmaster_intern_client) {
   ...
   component_set_thread_name(threadnames[thread_id] ? threadnames[thread_id] : prognames[progid]);
   ...
   if (feature_initialize_from_string(bootstrap_get_security_mode(), alpp)) {
   ...
   if (!gdi3_get_csp_path_obj()) {
      ...
      answer_list_add(alpp, SGE_EVENT, STATUS_ESEMANTIC, ANSWER_QUALITY_CRITICAL);
      DRETURN(AE_ERROR);
   }
...
*/
int sge_gdi2_setup(int component_id, u_long32 thread_id, lList **answer_list) {
   DENTER(TOP_LAYER);
   ...
   int ret = sge_setup2(component_id, thread_id, answer_list, false);
   if (ret != AE_OK) {
      DRETURN(ret);
   }

   if (sge_gdi_ctx_class_prepare_enroll(answer_list) != CL_RETVAL_OK) {
