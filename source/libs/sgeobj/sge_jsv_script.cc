...
jsv_split_commandline(const char *input, dstring *command, dstring *subcommand, dstring *args)
{
   ...
   if (input != nullptr) {
      ...
      if (token1 != nullptr) {
         ...
         token2 = sge_strtok_r(nullptr, " ", &cntx);
         if (token2 != nullptr) {
            bool first = true;    
            const char *arg = nullptr;

            sge_dstring_append(subcommand, token2);
            arg = sge_strtok_r(nullptr, " ", &cntx);
            while (arg != nullptr) {
               if (first) {
                  first = false;
               } else {
...
jsv_handle_param_command(lListElem *jsv, lList **answer_list, dstring *c, dstring *s, dstring *a)
{
   ...
   if (param != nullptr) {
      ...
      lListElem *new_job = (lListElem *)lGetRef(jsv, JSV_new_job);
         ...
         if (ret && strncmp("binding_exp_", param, strlen("binding_exp_")) == 0) {
            ...
            if (ret) {
               ...
               if (ret) {
                  ...
                  if (has_new_socket && new_socket_id + 1 > (u_long32)sockets) {
                     do_resize = true;
                     new_length = new_socket_id + 1;
                  }
                  if (has_new_core && new_core_id + 1 > (u_long32)cores) {
                     do_resize = true;
...
jsv_handle_result_command(lListElem *jsv, lList **answer_list, dstring *c, dstring *s, dstring *a)
{
   ...
   const char *sub_command = nullptr;
...
jsv_handle_started_command(lListElem *jsv, lList **answer_list, dstring *c, dstring *s, dstring *a)
{
   ...
   bool ret = true;
   lListElem *old_job = (lListElem *)lGetRef(jsv, JSV_old_job);
    ...
    */
   {
      ...
      if (hold_list != nullptr) {
         sge_dstring_clear(&buffer);
         sge_dstring_sprintf(&buffer, "%s h %c", prefix, (hold_list != nullptr) ? 'u' : 'n');
         jsv_send_command(jsv, answer_list, sge_dstring_get_string(&buffer));
      }
   }
    ...
    */
   {
      ...
      const char *strategy = ((binding != nullptr) ? lGetString(binding, BN_strategy) : nullptr);

      if (strategy != nullptr && strcmp(strategy, "no_job_binding") != 0) {
    ...
    */  
   {
      ...
      const lListElem *display = nullptr;
...
jsv_handle_env_command(lListElem *jsv, lList **answer_list, dstring *c, dstring *s, dstring *a)
{
   ...
   if (skip_check != true) {
      if (mod != nullptr && var != nullptr &&
          (((strcmp(mod, "MOD") == 0 || strcmp(mod, "ADD") == 0) && val != nullptr) ||
            (strcmp(mod, "DEL") == 0 && val == nullptr))) {
         lList *env_list = lGetListRW(new_job, JB_env_list);
         ...
         if (strcmp("ADD", mod) == 0 || strcmp("MOD", mod) == 0) {
            ...
            answer_list_add_sprintf(answer_list, STATUS_DENIED, ANSWER_QUALITY_ERROR,
                                    ...
                                    (mod != nullptr) ? mod : "<null>",
                                    (var != nullptr) ? var : "<null>",
   ...
   if (ret) {
      ...
      while (!lGetBool(jsv, JSV_done)) {
         if (sge_get_gmt() - start_time > jsv_timeout) {
            ...
            FILE *err_stream = (FILE *)lGetRef(jsv, JSV_err);
            FILE *out_stream = (FILE *)lGetRef(jsv, JSV_out);
   ...
   if (cull_attr == JB_execution_time) {
      ...
      if (job_name != nullptr) {
         if (job_name[0] == JOB_NAME_DEL) {
            ...
            if (help_str != nullptr && help_str[1] != '\0') {
               /* This is case D) */
               ret = "N";
            }
         } else {
   ...
   if (job != nullptr) {
      ...
      if (jsv_url && strcasecmp(jsv_url, "none") != 0) {
         ...
         if (jsv_allowed_mod && strcmp(jsv_allowed_mod, "none") != 0) {
            ...
            const lDescr *pointer = nullptr;
            lList *allowed_switches = nullptr;
            lList *got_switches = nullptr;
            const lListElem *allowed = nullptr;
