...
centry_rsmap_check_request_params(lList **answer_list, const lListElem *centry,
   ...
   for (const std::string &param : params) {
      ...
      if (centry_rsmap_is_reserved_param(param_name.c_str())) {
         ...
         if (param_name == RSMAP_REQUEST_PARAM_ID) {
            if (param_value.empty() ||
                sge_eval_expression(TYPE_CSTR, param_value.c_str(), "0", nullptr) < 0) {
               answer_list_add_sprintf(answer_list, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR,
      ...
      if (char_cep == nullptr) {
         ...
         ret = false;
      } else if (lGetUlong(char_cep, CE_valtype) == TYPE_RSMAP) {
 ...
 */
static bool
centry_rsmap_value_matches(u_long32 type, u_long32 relop, const lListElem *request,
      ...
      case TYPE_RESTR: {
         ...
         switch (type) {
            case TYPE_RESTR:
               ...
               cmp = sge_eval_expression(TYPE_RESTR, req_str, off_str, nullptr);
               break;
            case TYPE_CSTR:
...
centry_rsmap_instance_matches_properties(const lListElem *defined_ep, const lList *required) {
   ...
   for_each_ep (req_ep, required) {
      ...
      if (!centry_rsmap_value_matches(lGetUlong(req_ep, CE_valtype), lGetUlong(req_ep, CE_relop),
                                      req_ep, offer)) {
         return false;
      }
   }
...
centry_rsmap_instance_matches_id(const lListElem *defined_ep, const char *id_expr) {
   ...
   return sge_eval_expression(TYPE_CSTR, id_expr, id, nullptr) == 0;
}
...
centry_rsmap_best_free_group(const lListElem *resource_definition,
                             ...
                             u_long32 *free_amount, const char *id_expr,
                             const lList *required_props) {
...
centry_rsmap_take_instances(const lListElem *resource_definition,
                            ...
                            lList **selected, const char *id_expr, const lList *required_props) {
...
centry_rsmap_select_instances(const lListElem *resource_definition,
                              const lList *taken, const lList *already,
                              u_long32 amount, lList **selected, const char *id_expr,
                              const lList *required_props) {
...
centry_rsmap_best_free_id(const lListElem *resource_definition,
                          const lList *taken, u_long32 *free_amount, const char *id_expr,
                          const lList *required_props) {
