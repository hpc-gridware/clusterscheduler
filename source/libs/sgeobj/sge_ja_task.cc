...
bool ja_task_add_finished_pe_task(lListElem *ja_task, const char *pe_task_id)
{
   ...
   if (pe_task != nullptr) {
      INFO("already handled exit of pe task " SFQ " in ja_task " sge_uu32"\n", pe_task_id, lGetUlong(ja_task, JAT_task_number));
      DRETURN(false);
   }
...
bool ja_task_clear_finished_pe_tasks(lListElem *ja_task) {
   ...
   if (pe_task_list == nullptr) {
      DPRINTF("no finished pe task list to clear in ja_task " sge_uu32 "\n", lGetUlong(ja_task, JAT_task_number));
      DRETURN(false);
   }
   ...
   DPRINTF("cleared finished pe task list in ja_task " sge_uu32 "\n", lGetUlong(ja_task, JAT_task_number));
